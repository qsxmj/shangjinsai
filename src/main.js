...
function weekStart(date) { return addDays(new Date(date.getFullYear(), date.getMonth(), date.getDate()), -dayIndex(date)) }

function defaultItems(date) {
  const weekday = date.getDay()
  if (weekday === 6) return []
  if (weekday === 0) return []
  const stem = [1, 3, 5].includes(weekday) ? ['语文点线面', '化学错题/知识点收集整理', '数学/化学错题深研'] : ['英语教材深研', '数学错题/知识点收集整理', '数学/化学错题深研']
  return [
    ...
    { text: '', editable: true, placeholder: '今日生物主题', done: false, waived: false },
    { text: '物理错题/知识点收集整理', editable: false, done: false, waived: false },
    { text: '', editable: true, placeholder: '留空，或写下临时任务', done: false, waived: false },
    { text: '', editable: true, placeholder: '生物课本阅读研习', done: false, waived: false },
...
function renderHeader() {
  ...
  return `<header class="topbar">
    ...
    <button class="icon-btn" data-action="spin-open" aria-label="摸鱼大转盘">◌</button>
  </header>`
}
...
function renderTask(item, index, date) {
  ...
  return `<div class="task-row ${status}" data-index="${index}"><button class="check-btn" data-action="toggle-task" aria-label="${item.done ? '标记未完成' : '标记完成'}">${item.done ? '✓' : item.waived ? '✦' : item.flagged ? '!' : '○'}</button><span class="task-no">${index + 1}.</span>${disabled ? `<span class="task-text">${escape(item.text)}</span>` : `<input class="task-input" data-task-input value="${escape(item.text)}" placeholder="${escape(item.placeholder || '')}" />`}<span class="task-tail"></span></div>`
}
...
function renderWeek() {
  ...
  return `<section class="overview-page"><div class="overview-head"><div><span class="eyebrow">${dateLabel(start)} — ${fmt(addDays(start, 6))}</span><h1>一周 · 轻轻展开</h1></div><button class="text-btn" data-action="day-view">回到今日 ↗</button></div><div class="week-grid">${Array.from({ length: 7 }, (_, i) => { const d = addDays(start, i); return renderMiniDay(d) }).join('')}</div></section>`
}

function renderMiniDay(date) {
  const entry = entryFor(date)
  const items = date.getDay() === 0 ? [] : date.getDay() === 6 ? entry.saturdayItems.filter(x => x.text).slice(0, 3).map(x => x.text) : entry.items.filter(x => x.text).slice(0, 4).map(x => x.text)
  const done = date.getDay() === 6 ? entry.saturdayItems.filter(x => x.text && (x.done || x.waived)).length : entry.items.filter(x => x.text && (x.done || x.waived)).length
...
function renderMonth() {
  ...
  const cells = Array.from({ length: offset + days }, (_, i) => { if (i < offset) return '<div class="month-cell empty"></div>'; const d = new Date(year, month, i - offset + 1); const e = entryFor(d); const total = e.items.filter(x => x.text).length; const done = e.items.filter(x => x.text && (x.done || x.waived)).length; return `<button class="month-cell ${iso(d) === iso(state.date) ? 'selected' : ''} ${d.getDay() === 0 ? 'sunday' : ''}" data-date="${iso(d)}"><span>${d.getDate()}</span><i style="--p:${total ? done / total : 0}"></i><small>${d.getDay() === 0 ? '统计' : total ? `${done}/${total}` : '—'}</small></button>` })
...
function weekDates(start) { return Array.from({ length: 6 }, (_, i) => addDays(start, i)) }
function statsFor(start) {
  ...
  dates.forEach(d => { const e = entryFor(d); const saturday = d.getDay() === 6; const source = saturday ? e.saturdayItems : e.items; words += source.filter(x => x.text).map(x => x.text).join('').length; journalWords += e.journal.length; source.forEach((x, i) => { if (!x.text) return; planned++; if (x.done || x.waived || (saturday && e.waivedSaturday)) done++; else { unfinished.push({ date: d, text: x.text, index: i }); if (!saturday && i !== 5) { let subject = ''; if (i < 3) subject = [1, 3, 5].includes(d.getDay()) ? ['语文', '化学', '化学'][i] : ['英语', '数学', '数学'][i]; else if (i === 3 || i === 6) subject = '生物'; else if (i === 4 || i === 7) subject = '物理'; if (subject) subjects[subject]++ } } }) })
...
function renderWeeklyStats(start) {
  ...
  return `<section class="stats-page"><div class="stats-head"><div><span class="eyebrow">${dateLabel(start)} — ${fmt(addDays(start, 6))}</span><h1>本周回望</h1></div><button class="text-btn" data-action="day-view">回到日视图 ↗</button></div><div class="blessing-row">${renderBlessing(start)}</div><div class="stat-hero"><div class="ring" style="--p:${percent}%"><span>${percent}<small>%</small></span></div><div><div class="stat-caption">完成率</div><div class="stat-big">${s.done}<small> / ${s.planned} 项</small></div><div class="stat-sub">${s.unfinished.length ? `还有 ${s.unfinished.length} 项等待你` : '本周全部完成，漂亮。'}</div></div></div><div class="stat-cards"><div><span>计划字数</span><b>${s.words}</b></div><div><span>日结字数</span><b>${s.journalWords}</b></div><div><span>未完成</span><b class="danger">${s.unfinished.length}</b></div></div><div class="subject-board"><div class="section-label"><span>未完成分布</span><span class="line"></span><span class="muted">按学科</span></div><div class="subject-bars">${Object.entries(s.subjects).map(([k,v]) => `<div><span>${k}</span><i><em style="width:${s.planned ? Math.min(100, v / Math.max(1, s.planned) * 100 * 3) : 0}%"></em></i><b>${v}</b></div>`).join('')}</div></div><div class="unfinished-list"><div class="section-label"><span>仍在等待的事</span><span class="line"></span></div>${s.unfinished.length ? s.unfinished.map(x => `<div class="unfinished-item"><span>${fmt(x.date)}</span><b>${escape(x.text)}</b></div>`).join('') : '<div class="empty-state">这一周没有遗憾。</div>'}</div></section>`
}
...
function applyPrize(result) {
  ...
  if (result === '免下一周工作日') { const monday = nextDateMatching(d => d.getDay() === 1); if (!monday) return; const targets = Array.from({ length: 5 }, (_, i) => { const d = new Date(monday); d.setDate(d.getDate() + i); return d }).filter(within); targets.forEach(d => entryFor(d).items.forEach(x => { x.waived = true; x.done = false })); recordPrize('整周免单', result, targets); return }
...
function bind() {
  document.querySelectorAll('[data-action]').forEach(btn => btn.addEventListener('click', () => {
    ...
    else if (action === 'pick-date') { const input = prompt('输入日期（如 2026-07-21）', iso(state.date)); if (input && within(parse(input))) state.date = parse(input); }
    ...
    else if (action === 'day-view') state.view = 'day'
    else if (action === 'week-view') state.view = 'week'
    else if (action === 'month-view') state.view = 'month'
    else if (action === 'spin-open') return showSpin()
    else if (action === 'spin') return spin()
    else if (action === 'close-modal') return closeModal()
    else if (action === 'complete-paper') { const e = entryFor(state.date); if ((state.data.paperRewards || 0) < 3) { state.data.paperRewards = (state.data.paperRewards || 0) + 1; e.spin.extra++; save(); toast('已获得 1 次额外转盘机会') } else toast('整个假期最多额外 3 次') }
    render()
  }))
  ...
  document.querySelector('[data-journal]')?.addEventListener('input', e => persistField('journal', e.target.value))
  document.querySelector('[data-goal]')?.addEventListener('input', e => persistField('goal', e.target.value))
  document.querySelectorAll('[data-date]').forEach(el => el.addEventListener('click', () => { state.date = parse(el.dataset.date); state.view = 'day'; render() }))
